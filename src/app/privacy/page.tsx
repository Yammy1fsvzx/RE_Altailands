        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 sm:p-8 md:p-10 space-y-8">
          ...
          <ScrollAnimation>
            <section className="space-y-4">
              ...
              <ul className="list-disc list-inside text-gray-600 space-y-2 ml-4">
                {contactInfo?.email && (
                  <li>По электронной почте: <a href={`mailto:${contactInfo.email}`} className="text-primary hover:text-primary-dark transition-colors">{contactInfo.email}</a></li>
                )}
                {contactInfo?.phone && (
                  <li>По телефону: <a href={`tel:${contactInfo.phone}`} className="text-primary hover:text-primary-dark transition-colors">{formatPhoneNumber(contactInfo.phone)}</a></li>
                )}
